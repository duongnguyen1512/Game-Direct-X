...
int GameInit(HINSTANCE hInstance, HWND hWnd) 
{
	...
	GTexture *texture = new GTexture(d3ddv, "bat.png");
...
void GameRun(HWND hWnd)
{
	if (d3ddv->BeginScene()) 
	{
		...
		//MOVE THE RECT
		if (KEYDOWN(VK_UP) && !CheckCollision())
			rect->y -= SPEED;
		if (KEYDOWN(VK_DOWN) && !CheckCollision())
			rect->y += SPEED;
		if (KEYDOWN(VK_LEFT) && !CheckCollision())
			rect->x -= SPEED;
		if (KEYDOWN(VK_RIGHT) && !CheckCollision())
			rect->x += SPEED;
